...
public class ListaBinaria {
    ...
    // Contrutor da lista
    public ListaBinaria(int tamanho) {
        this.array = new ListaBinaria[tamanho];
        this.n = 0;
    }
    ...
    public ListaBinaria(int app_id, String name, Date release_date, String owners, int age, float price, int dlcs,
    ...
    // Metodo read string
    public void read(String str, ListaBinaria jogos[], int cont) throws Exception {
        int i = 0;
    ...
    public static String[] converterStringParaArray1(String str, ListaBinaria jogos[], int cont) {
    ...
    public static String[] converterStringParaArray2(String str, ListaBinaria jogos[], int cont) {
        String array[] = new String[50];
        int j = 0;
        if (str == null) {
            array[0] = "null";
    ...
    // Metodo para descobrir o id pela String
    public static int descobrirIdGame(String str, int cont, ListaBinaria jogos[]) {
        int id = 0;
        for (int i = 0; i < cont; i++) {
            String palavra = jogos[i].getName();
            if (palavra.compareTo(str) == 0) {
                id = jogos[i].getApp_id();
                i = cont;
            }
        }
        return id;
    ...
    public void inserirFim(ListaBinaria game) throws Exception {
    ...
    // Pesquisa Binaria
    public void pesqBin(int id) {
        boolean retorno = false;
        int dir = n, esq = 0, meio;
        while (esq <= dir) {
            meio = (esq + dir) / 2;
            if (id == array[meio].getApp_id()) {
                retorno = true;
                esq = dir + 1;
            } else if (id > array[meio].getApp_id()) {
                esq = meio + 1;
            } else {
                dir = meio - 1;
            }
    ...
    // Metodo para ordenar o array [Usando Quicksort]
    public void sort() {
        quicksort(0, n - 1);
    }
    ...
    private void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        int pivo = array[(dir + esq) / 2].getApp_id();
        while (i <= j) {
            while (array[i].getApp_id() < pivo)
                i++;
            while (array[j].getApp_id() > pivo)
                j--;
            if (i <= j) {
                swap(i, j);
                i++;
    ...
    public void swap(int i, int j) {
        int valor = array[i].getApp_id();
        array[i].setApp_id(j);
        array[j].setApp_id(valor);
    }

    public static void main(String[] args) throws Exception {
        long start = System.currentTimeMillis();
        long end;
        BufferedWriter arq = new BufferedWriter(new FileWriter("matrícula_binaria.txt"));
        ...
        ListaBinaria jogos[] = new ListaBinaria[4500];
        String line = "";
        line = br.readLine();
        while (line != null) {
            jogos[i] = new ListaBinaria();
        ...
        // Instanciar uma lista de Games
        ListaBinaria list = new ListaBinaria(i);

        // Inserir Valores na Lista
        int cont = 0;
        while (!isFim(val[cont])) {
            int j = 0;
            while (j < 4403 && jogos[j].getApp_id() != converterStringParaInt(val[cont])) {
                j++;
            }
            list.inserirFim(jogos[j]);
            cont++;
        }
        // Ordenar o array
        list.sort();

        // Pesquisar pelo jogo na lista
        i = 0;
        String entrada[] = new String[100];
        entrada[i] = MyIO.readLine();
        while (!isFim(entrada[i])) {
            int id = descobrirIdGame(entrada[i], contJogos, jogos);
            list.pesquisar(id);
            i++;
            entrada[i] = MyIO.readLine();
        }
